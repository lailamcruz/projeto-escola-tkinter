...
def adicionar_aluno() -> None:
    ...
    matricula_atual += 1
    aluno = {"matricula": matricula_atual, "nome": nome, "curso": curso, "nota": nota}
    alunos.append(aluno)
    limparCampos()
...
def atualizarTabela() -> None:
    ...
    for aluno in alunos:
        if busca in aluno['nome'].lower():
            tabela_alunos.insert("", END, values=(aluno["matricula"], aluno["nome"], aluno["curso"], aluno["nota"]))
...
def deletarAluno() -> None:
    ...
    if opcao:
        ...
        limparCampos()
        atualizarTabela()
        messagebox.showinfo("Deletado!", "Aluno deletano com sucesso!")
    else:
        limparCampos()
...
def editarAluno() -> None:
    ...
    opcao = messagebox.askyesno("tem certeza amor?", "deseja alterar os dados do aluno?")
...
label_matricula = Label(janela, text="Matrícula:",  font="Arial 14 bold", fg="red")
...
cb_curso = ttk.Combobox(janela, width=25, values=["Javascript", "Python", "Django", "React"], font="tahoma 14", state="readonly")
...
btn_adicionar = Button(janela, fg="red", bg="white", text="Adicionar", font="Tahoma 12 bold", command=adicionar_aluno)
btn_adicionar.grid(row=4, column=0)
btn_editar = Button(janela, fg="red", bg="white", text="Editar", font="Tahoma 12 bold", command=editarAluno)
btn_editar.grid(row=4, column=1)
btn_excluir = Button(janela, fg="red", bg="white", text="Excluir", font="Tahoma 12 bold", command=deletarAluno)
btn_excluir.grid(row=4, column=2)

columns = ("Matrícula", "Nome", "Curso", "Nota")
